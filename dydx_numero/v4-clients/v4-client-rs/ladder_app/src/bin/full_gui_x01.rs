...
// No direct dYdX indexer websockets here.
//
// - Live mode:
...
//   - Time display: Unix vs Local (x-axis + labels)
//   - Mouse drag/zoom on candles and volume
//   - Shift + scroll over candles/volume => vertical zoom
//   - Current candle kept roughly centered horizontally
//   - Simple theme + layout toggles, easy to extend
//
...
fn compute_snapshot_for(data: &TickerData, target_ts: u64) -> Snapshot {
    ...
    // prepare candle engines for all TFs we care about
    ...
    for e in &data.book_events {
        ...
        // Update candles only if both sides exist
    ...
    // pick 1m (60s) for last mid/vol if available, else smallest tf
    let ref_tf = 60u64;
    let (last_mid, last_vol) = if let Some(series) = candles.get(&ref_tf).and_then(|v| v.last()) {
...
impl ComboApp {
    ...
    fn tick_live(&mut self) {
        ...
        // clone TickerData so we don't hold an immutable borrow of self while mutating self
                ...
                |ui| {
                    for t in &tickers {
                        ...
                        if ui.selectable_label(selected, t).clicked() {
                            ...
                            // clone td so we don't hold an immutable borrow of self
    ...
    fn ui_live(&mut self, ui: &mut egui::Ui) {
        ...
        // Own a local clone of the snapshot so closures don't borrow &self
