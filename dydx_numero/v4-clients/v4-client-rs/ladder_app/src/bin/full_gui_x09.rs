...
impl ComboApp {
    ...
    fn reload_all_tickers(&mut self) {
        ...
        if let Some(td_ref) = self.current_ticker_data() {
            let min_ts = td_ref.min_ts;
            let max_ts = td_ref.max_ts;
    ...
    fn maybe_reload_csvs(&mut self) {
        let interval = Duration::from_secs_f64(self.reload_interval_secs.max(1.0));
    ...
    fn run_script(&mut self, snap: &Snapshot) {
        ...
        ctx.insert("tf".into(), (self.chart.selected_tf as i64).into());
        ctx.insert("history".into(), (self.chart.show_candles as i64).into());
        ...
        ctx.insert("show_ladders".into(), self.layout.show_ladders.into());
        ctx.insert("show_trades".into(), self.layout.show_trades.into());
        ctx.insert("show_volume".into(), self.layout.show_volume.into());
        ctx.insert("reload_secs".into(), self.reload_interval_secs.into());
        ctx.insert(
            "mode".into(),
            match self.mode {
                Mode::Live => "live",
    ...
    fn send_manual_order(&mut self, side: OrderSide) {
        ...
        let size_bd = match BigDecimal::from_str(&s_str) {
            Ok(bd) => bd,
            Err(e) => {
                self.last_order_msg = format!("Invalid size: {s_str} ({e})");
                return;
            }
        };
    ...
    fn send_bot_order(&mut self, side: OrderSide) {
        ...
        if size_val <= 0.0 {
            self.last_order_msg = "Bot size <= 0; nothing to send".to_string();
            return;
        }
        ...
        let size_bd = match BigDecimal::from_str(&s_str) {
            Ok(bd) => bd,
            Err(e) => {
                self.last_order_msg = format!("Bot size invalid: {s_str} ({e})");
                return;
            }
        };
