// ladder_app/src/bin/gui_app1.rs
//
...
//   egui_plot 0.27
//
// Candles are drawn manually as:
//   - wick: thin vertical line (low->high)
//   - body: thicker vertical line (open->close)
//
// Run:
//   cargo run -p ladder_app --bin gui_app1
...
impl MyApp {
    ...
    fn ui_trading_panel(&mut self, ui: &mut egui::Ui) {
        ui.group(|ui| {
            ...
            ui.label(format!("Mark price: {:.2}", self.last_price));
            ui.separator();
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                ui.horizontal(|ui| {
                    ui.vertical(|ui| {
                        ui.label("Bids");
                        egui::Grid::new("bids_grid").striped(true).show(ui, |ui| {
                    ...
                    ui.vertical(|ui| {
                        ui.label("Asks");
                        egui::Grid::new("asks_grid").striped(true).show(ui, |ui| {
                            ui.label("Price"); ui.label("Size"); ui.end_row();
    ...
    fn ui_candles(&mut self, ui: &mut egui::Ui) {
        ...
        // ---- Candle plot ----
        ui.group(|ui| {
            ui.label("Candles (manual OHLC drawing)");
            Plot::new("candles_plot")
                .height(300.0)
                .include_y(y_min)
                .include_y(y_max)
                .show(ui, |plot_ui| {
                    // draw each candle as two vertical lines
                    for c in visible {
                        let x = c.t as f64;

                        // wick
                        let wick_pts: PlotPoints = vec![[x, c.low], [x, c.high]].into();
                        let wick = Line::new(wick_pts);

                        // body (thicker vertical line between open & close)
                        let body_pts: PlotPoints = vec![[x, c.open], [x, c.close]].into();
                        let mut body = Line::new(body_pts);

                        // try to make body thicker; if this ever errors in your version,
                        // just delete the next line.
                        body = body.width(3.0);

                        // color by direction (optional)
                        let color = if c.close >= c.open { Color32::GREEN } else { Color32::RED };
                        let wick = wick.color(color);
                    ...
                    if let Some(last) = visible.last() {
                        plot_ui.hline(HLine::new(last.close).name("now_px"));
                        plot_ui.vline(VLine::new(last.t as f64).name("now_t"));
                    }
        ...
        // ---- RSI plot ----
        let closes: Vec<f64> = series_vec.iter().map(|c| c.close).collect();
        let rsi_pts = compute_rsi(&closes, 14);
        ...
        ui.group(|ui| {
            ...
            Plot::new("rsi_plot")
                ...
                .show(ui, |plot_ui| {
                    if !rsi_pts.is_empty() {
                        let pts: PlotPoints = rsi_pts.iter().map(|(i,v)| [*i, *v]).collect::<Vec<_>>().into();
