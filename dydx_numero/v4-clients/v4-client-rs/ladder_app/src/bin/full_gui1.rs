// ladder_app/src/bin/gui_app_combo.rs
//
...
//   - Y-axis control: Auto vs manual min/max, plus mouse drag/zoom
//   - Current candle kept centered horizontally (TF changes respect position)
//
// Run (no mnemonic needed for this combined app):
//   cargo run -p ladder_app --bin gui_app_combo
//
...
// use integer keys so BTreeMap ordering works nicely
type PriceKey = i64;
...
impl ComboApp {
    ...
    fn tick_live(&mut self) {
        // update book from async feed if changed
        ...
        if let Some(mid) = self.live_book.mid() {
            // no actual trade-volume stream yet, so approximate volume per tick
            let vol = 0.0;
            ...
            ui.menu_button(format!("Ticker: {}", self.current_ticker), |ui| {
                for t in &tickers {
                    ...
                    if ui.selectable_label(selected, t).clicked() {
                        ...
                        // if replay data exists for it, snap to max_ts
    ...
    fn ui_live(&mut self, ui: &mut egui::Ui) {
        // book + candles share same live_book / candle series
        ...
        // live candles + volume (no trades in live yet)
        ...
        ui.allocate_ui(egui::vec2(avail_w, volume_h), |ui| {
            ...
            .show(ui, |plot_ui| {
                ...
                if let Some(snap) = snap {
                    let _ = snap; // reserved if you want overlays later
                }
            });
        });
    }
...
fn main() {
    ...
    // watch channel for ticker selection in async feed
    ...
    let app = ComboApp::new(book_rx, replay_data);

    // pass ticker_tx into the app by putting it into global state if you want
    // dynamic resub from UI later; for now ticker in live feed defaults to ETH-USD.
