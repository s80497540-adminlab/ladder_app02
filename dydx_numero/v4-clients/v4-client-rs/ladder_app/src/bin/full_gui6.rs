// ladder_app/src/bin/full_gui5.rs
//
...
//   - Persists candles to CSV and reloads them on restart
//
// Replay mode:
...
//
// Run:
//   cargo run -p ladder_app --bin full_gui5
//
...
struct ChartSettings {
    ...
    x_pan_secs: f64,
    selected_tf: u64,
    // how fat the candles are relative to TF (0.2 - 1.0)
    candle_body_frac: f64,
}
...
struct LayoutSettings {
    ladders_height_frac: f32, // fraction of central panel height
    depth_width_frac: f32,    // fraction of top row width for depth chart
    ladder_levels: usize,     // how many levels to show in ladders
}

impl Default for LayoutSettings {
    fn default() -> Self {
        Self {
            ladders_height_frac: 0.35,
            depth_width_frac: 0.45,
            ladder_levels: 20,
        }
    }
}
...
struct ComboApp {
    ...
    live_last_ts: u64,
    // for persistence flush throttling
    last_candle_flush_ts: u64,
...
impl ComboApp {
    fn new(
        ...
        // load any previously saved live candles for default ticker
        app.load_live_candles_for_current();
        app
    }
    ...
    fn theme_colors(&self) -> ThemeColors {
        match self.theme {
            ThemeChoice::Classic => ThemeColors {
                candle_up: Color32::from_rgb(80, 200, 120),
                candle_down: Color32::from_rgb(230, 80, 80),
                depth_bid: Color32::from_rgb(0, 180, 0),
                depth_ask: Color32::from_rgb(200, 0, 0),
                volume: Color32::from_rgb(120, 170, 240),
            },
            ThemeChoice::DarkNeon => ThemeColors {
                candle_up: Color32::from_rgb(0, 255, 180),
                candle_down: Color32::from_rgb(255, 80, 200),
                depth_bid: Color32::from_rgb(0, 220, 120),
                depth_ask: Color32::from_rgb(255, 120, 0),
                volume: Color32::from_rgb(180, 180, 255),
            },
            ThemeChoice::Ocean => ThemeColors {
                candle_up: Color32::from_rgb(60, 200, 255),
                candle_down: Color32::from_rgb(0, 90, 160),
                depth_bid: Color32::from_rgb(0, 180, 200),
                depth_ask: Color32::from_rgb(0, 80, 140),
                volume: Color32::from_rgb(140, 210, 255),
            },
            ThemeChoice::HighContrast => ThemeColors {
                candle_up: Color32::from_rgb(0, 255, 0),
                candle_down: Color32::from_rgb(255, 0, 0),
                depth_bid: Color32::from_rgb(0, 255, 0),
                depth_ask: Color32::from_rgb(255, 0, 0),
                volume: Color32::from_rgb(255, 255, 0),
            },
            ThemeChoice::Mono => ThemeColors {
                candle_up: Color32::from_rgb(200, 200, 200),
                candle_down: Color32::from_rgb(120, 120, 120),
                depth_bid: Color32::from_rgb(180, 180, 180),
                depth_ask: Color32::from_rgb(220, 220, 220),
                volume: Color32::from_rgb(160, 160, 160),
            },
        }
    }
    ...
    fn candle_path(ticker: &str, tf_secs: u64) -> String {
        // normalize ticker for filename
        let safe = ticker.replace('/', "-");
        format!("data/candles_{}_{}s.csv", safe, tf_secs)
    }

    fn load_live_candles_for_ticker(&mut self, ticker: &str) {
        let p30 = Self::candle_path(ticker, 30);
        let p60 = Self::candle_path(ticker, 60);
        let p180 = Self::candle_path(ticker, 180);
        let p300 = Self::candle_path(ticker, 300);

        self.live_tf_30s.load_from_csv(&p30);
        self.live_tf_1m.load_from_csv(&p60);
        self.live_tf_3m.load_from_csv(&p180);
        self.live_tf_5m.load_from_csv(&p300);

        // adjust last_ts from last candle if any
        let all_series = self.live_series().to_vec();
        if let Some(last) = all_series.last() {
            self.live_last_ts = last.t;
        }
    }

    fn save_live_candles_for_ticker(&self, ticker: &str) {
        let dir = Path::new("data");
        let _ = std::fs::create_dir_all(dir);
        ...
        self.live_tf_30s.save_to_csv(&p30);
        self.live_tf_1m.save_to_csv(&p60);
        self.live_tf_3m.save_to_csv(&p180);
        self.live_tf_5m.save_to_csv(&p300);
    }

    fn load_live_candles_for_current(&mut self) {
        let t = self.current_ticker.clone();
        self.load_live_candles_for_ticker(&t);
    }

    fn save_live_candles_for_current(&self) {
        let t = self.current_ticker.clone();
        self.save_live_candles_for_ticker(&t);
    }

    fn on_ticker_changed(&mut self) {
        // reset live candle aggregators and load from disk for new ticker
        self.live_tf_30s = CandleAgg::new(30);
        self.live_tf_1m = CandleAgg::new(60);
        self.live_tf_3m = CandleAgg::new(180);
        self.live_tf_5m = CandleAgg::new(300);
        self.last_candle_flush_ts = 0;

        self.load_live_candles_for_current();
    }
    ...
    fn live_series(&self) -> &[Candle] {
        match self.chart.selected_tf {
            30 => self.live_tf_30s.series(),
            60 => self.live_tf_1m.series(),
            180 => self.live_tf_3m.series(),
            300 => self.live_tf_5m.series(),
            _ => self.live_tf_1m.series(),
        }
    }
    ...
    fn tick_live(&mut self) {
        ...
        if let Some(mid) = self.live_book.mid() {
            // we don't have real trade volume here yet, so treat each update as 1 unit
            let vol = 1.0;
        ...
        // flush candles to disk at most once per second when updated
        if updated && ts != self.last_candle_flush_ts {
            self.last_candle_flush_ts = ts;
            self.save_live_candles_for_current();
        }
    }
    ...
    fn ui_top_bar(&mut self, ui: &mut egui::Ui) {
            ...
            ui.menu_button(format!("Ticker: {}", self.current_ticker), |ui| {
                for t in &tickers {
                    ...
                    if ui.selectable_label(selected, t).clicked() {
                        ...
                        // reload persistent candles for new ticker
        ...
        // customization: theme + layout + candle width
        ui.horizontal(|ui| {
            // theme picker
            ui.label("Theme:");
            ui.selectable_value(&mut self.theme, ThemeChoice::Classic, "Classic");
            ui.selectable_value(&mut self.theme, ThemeChoice::DarkNeon, "Dark Neon");
            ui.selectable_value(&mut self.theme, ThemeChoice::Ocean, "Ocean");
            ui.selectable_value(&mut self.theme, ThemeChoice::HighContrast, "High Contrast");
            ui.selectable_value(&mut self.theme, ThemeChoice::Mono, "Mono");

            ui.separator();

            ui.collapsing("Layout / Style", |ui| {
                ui.label("Top section layout:");
                ui.add(
                    egui::Slider::new(
                        &mut self.layout.ladders_height_frac,
                        0.2..=0.7,
                    )
                    .text("Ladders height"),
                );
                ui.add(
                    egui::Slider::new(
                        &mut self.layout.depth_width_frac,
                        0.3..=0.7,
                    )
                    .text("Depth width"),
                );
                ui.add(
                    egui::Slider::new(&mut self.layout.ladder_levels, 5..=60)
                        .text("Ladder levels"),
                );
                ...
                ui.label("Candles:");
                ui.add(
                    egui::Slider::new(&mut self.chart.candle_body_frac, 0.2..=1.0)
                        .text("Body width"),
                );
            });
        });
    ...
    fn ui_live(&mut self, ui: &mut egui::Ui) {
        // clone into a local Vec to avoid holding &self borrow across UI
        let series_vec: Vec<Candle> = self.live_series().to_vec();
        ...
        let ladders_h = avail_h * self
            .layout
            .ladders_height_frac
            .clamp(0.2, 0.7);

        ui.allocate_ui(egui::vec2(avail_w, ladders_h), |ui| {
            let depth_w = avail_w * self.layout.depth_width_frac.clamp(0.3, 0.7);
        ...
        // use the cloned Vec here; no borrow from &self
    ...
    fn ui_replay(&mut self, ui: &mut egui::Ui) {
        ...
        match self.replay_tab {
            ...
            ReplayTab::Candles => {
                // clone into local Vec to avoid holding &self borrow
                let series_vec: Vec<Candle> = self.replay_series(snap).clone();
    ...
    fn ui_candles_generic(
        ...
        let body_frac = self.chart.candle_body_frac.clamp(0.1, 1.0);
        let body_half = tf * body_frac * 0.5;
        ...
        ui.allocate_ui(egui::vec2(avail_w, candles_h), |ui| {
            ...
            .show(ui, |plot_ui| {
                ...
                for c in visible {
                    ...
                    let left = mid - body_half;
                    let right = mid + body_half;
