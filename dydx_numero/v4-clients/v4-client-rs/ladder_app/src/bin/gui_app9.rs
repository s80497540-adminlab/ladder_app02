// ladder_app/src/bin/gui_app8.rs
//
// Egui desktop app with live dYdX v4 testnet orderbook + candles + fake trading,
// and a minimal REAL testnet order sender (optional).
//
// Run:
//   export DYDX_TESTNET_MNEMONIC='mirror actor ... wait'
//   cargo run -p ladder_app --bin gui_app8
//
// Notes:
// - Uses candle_agg.rs (same module you already have).
// - Real trading is testnet-only here. Keep it that way until you're ready.
...
// dYdX client imports (workspace path dep)
...
struct MyApp {
    ...
    // real trading sender
    real_tx: mpsc::UnboundedSender<RealCmd>,
    real_enabled: bool,
    real_size: f64,
    ...
    fn ui_trading_panel(&mut self, ui: &mut egui::Ui) {
        ui.group(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Deposit → Margin").clicked() {
                ...
                if ui.button("Withdraw → Wallet").clicked() {
            ...
            ui.horizontal(|ui| {
                ...
                // smaller increments for TP/SL
                let step = 0.1;
                if ui.button("TP +0.1").clicked() {
                    self.trading.bump_tp(self.last_price, step);
                }
                if ui.button("TP -0.1").clicked() {
                    self.trading.bump_tp(self.last_price, -step);
                }
                if ui.button("SL +0.1").clicked() {
                    self.trading.bump_sl(self.last_price, step);
                }
                if ui.button("SL -0.1").clicked() {
                    self.trading.bump_sl(self.last_price, -step);
                }
            ...
            ui.separator();
            ui.heading("REAL testnet order (optional)");
            ui.checkbox(&mut self.real_enabled, "Enable real testnet sends");
            ui.add(
                egui::DragValue::new(&mut self.real_size)
                    .speed(0.001)
            ...
            ui.horizontal(|ui| {
                if ui.button("REAL Market BUY").clicked() && self.real_enabled {
                    let _ = self.real_tx.send(RealCmd::MarketBuy { size: self.real_size });
                }
                if ui.button("REAL Market SELL").clicked() && self.real_enabled {
                    let _ = self.real_tx.send(RealCmd::MarketSell { size: self.real_size });
                }
            });
        });
    }
            ...
            Plot::new("candles_plot")
                ...
                .show(ui, |plot_ui| {
                    ...
                    for c in visible {
                        ...
                        // candle body rectangle edges (flush by time width)
...
// ====== MAIN: spawn testnet feeds + real order executor + run GUI ======
use dydx_client::config::ClientConfig;
use dydx_client::indexer::{Feeds, IndexerClient, OrdersMessage, Ticker, Feed as DxFeed};
...
fn main() {
    ...
    // command channel for real orders
    let (real_tx, mut real_rx) = mpsc::unbounded_channel::<RealCmd>();
    ...
    // --- spawn real trading executor (testnet) ---
    rt.spawn(async move {
        use std::env;
        ...
        let wallet = Wallet::from_mnemonic(&mnemonic).expect("mnemonic ok");

        let mut node = NodeClient::connect(config.node).await.expect("node connect");
        let indexer = IndexerClient::new(config.indexer);

        let mut account = wallet.account(0, &mut node).await.expect("account");
        let sub = account.subaccount(0).expect("subaccount");

        // ---- IMPORTANT FIX: clone before moving into feeds ----
        let sub_for_feed = sub.clone();

        let mut feeds = indexer.clone().feed(); // cheap clone ok
        let _sub_feed = feeds
            .subaccounts(sub_for_feed, false)
            .await
            .expect("subaccounts feed");

        // process real order commands
        while let Some(cmd) = real_rx.recv().await {
                ...
                .await
            {
                Ok(m) => m,
                Err(e) => {
                    eprintln!("market fetch failed: {e}");
                    continue;
                }
            };

            let current_block_height = match node.latest_block_height().await {
                Ok(h) => h,
                Err(e) => {
                    eprintln!("block height failed: {e}");
                    continue;
                }
            };

            let (side, size_f) = match cmd {
                RealCmd::MarketBuy { size } => (OrderSide::Buy, size),
                RealCmd::MarketSell { size } => (OrderSide::Sell, size),
            };

            let size = match BigDecimal::from_str(&format!("{size_f}")) {
                Ok(s) => s,
                Err(e) => {
                    eprintln!("bad size: {e}");
                    continue;
                ...
                .build(123456)
            {
                Ok(x) => x,
                Err(e) => {
                    eprintln!("build order failed: {e}");
                    continue;
                }
            };

            match node.place_order(&mut account, order).await {
                Ok(txh) => println!("REAL testnet tx hash: {txh:?}"),
                Err(e) => eprintln!("place_order failed: {e}"),
            }
        }
    });
    ...
    if let Err(e) = eframe::run_native(
        "Ladder GUI (egui)",
        options,
        Box::new(|_cc| Box::new(MyApp::new(book_rx, real_tx))),
    ) {
