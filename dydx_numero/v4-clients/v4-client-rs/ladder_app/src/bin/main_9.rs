...
#[derive(Clone, Debug)]
struct TradingState {
    margin: f64,    // fake margin in USD
    leverage: f64,  // fake leverage
    position: f64,  // fake position size (contracts or units)
    side: PositionSide,
}
...
fn main() -> Result<()> {
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            match selected_tab {
                Tab::Orderbook => draw_orderbook_view(f, layout[1], &order_book, &trading),
                Tab::Candles => draw_candles_view(f, layout[1], &series, selected_tf, &chart, &trading),
            }
        })?;
...
fn trading_widget(trading: &TradingState) -> Paragraph<'static> {
    let side = trading.side.label();
    let notional = trading.notional();

    let lines = vec![
        Line::from(format!(
            "Side: {:<5} | Pos: {:>7.3} | Lev: {:>5.2}x | Margin: ${:>8.2} | Notional: ${:>9.2}",
            side,
            trading.position,
            trading.leverage,
            trading.margin,
        ...
        Line::from("Controls: b=LONG s=SHORT f=FLAT  9/0=margin -/=pos [/]=lev"),
    ];
...
fn draw_orderbook_view(
    ...
    let trade_widget = trading_widget(trading);
