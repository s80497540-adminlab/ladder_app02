// ladder_app/src/bin/gui_app11.rs
//
...
//  - Horizontal stretch/squeeze (x_zoom) + pan based on anchor_frac
//  - Last candle stays at the same screen position when switching TFs
...
//  - REAL testnet order placement buttons + subaccount feed snapshot
//
// Run:
//   export DYDX_TESTNET_MNEMONIC='...'
//   cargo run -p ladder_app --bin gui_app11
...
use dydx_client::indexer::{
    Feeds, Feed as DxFeed, IndexerClient, OrdersMessage, OrderbookResponsePriceLevel, Price,
...
struct ChartSettings {
    ...
    // horizontal zoom + anchor
    x_zoom: f64,      // >1 zoom-in (stretch), <1 zoom-out (squeeze)
    anchor_frac: f64, // [0,1] fraction from left where last candle's center sits
}
...
impl MyApp {
    fn new(
        ...
        Self {
            ...
            chart: ChartSettings {
                ...
                auto_scale: true,
                x_zoom: 1.0,
                anchor_frac: 0.75, // last candle at ~3/4 of the screen
            },
    ...
    fn tick(&mut self) {
        ...
        // synthetic volume per tick for now (1.0). Later: plug real trade sizes.
        let vol_tick = 1.0;

        // update candle aggs
        self.tf_30s.update(ts, self.last_price, vol_tick);
        self.tf_1m.update(ts, self.last_price, vol_tick);
        self.tf_3m.update(ts, self.last_price, vol_tick);
        self.tf_5m.update(ts, self.last_price, vol_tick);
    }
    ...
    fn ui_top_bar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            // Ticker selection combo (popup inline)
            egui::ComboBox::from_label("Ticker")
                .selected_text(self.trading.real_ticker.clone())
                .show_ui(ui, |ui| {
                    for sym in ["ETH-USD", "BTC-USD", "SOL-USD"] {
                        ui.selectable_value(
                            &mut self.trading.real_ticker,
                            sym.to_string(),
                            sym,
                        );
                    }
    ...
    fn ui_trading_panel(&mut self, ui: &mut egui::Ui) {
        ui.group(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Ticker:");
                ui.text_edit_singleline(&mut self.trading.real_ticker);
                ui.label("Size (units):");
                ui.add(
            ...
            ui.horizontal(|ui| {
                if ui.button("Market BUY").clicked() {
                    let t = self.trading.real_ticker.trim().to_string();
    ...
    fn ui_candles(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.horizontal(|ui| {
                if ui.button("← Pan").clicked() {
                    self.chart.anchor_frac =
                        (self.chart.anchor_frac - 0.05).clamp(0.05, 0.95);
                }
                if ui.button("Pan →").clicked() {
                    self.chart.anchor_frac =
                        (self.chart.anchor_frac + 0.05).clamp(0.05, 0.95);
        ...
        // layout: candles, volume, RSI, bottom info
        let candles_h = avail_h * 0.5;
        let volume_h = avail_h * 0.15;
        let rsi_h = avail_h * 0.2;
        let bottom_h = avail_h * 0.15;

        // common X window: last candle center anchored at anchor_frac
        let tf = self.selected_tf as f64;
        let last = visible.last().unwrap();
        let last_center = last.t as f64 + tf * 0.5;

        let base_span = tf * self.chart.show_candles as f64;
        let span = base_span / self.chart.x_zoom.max(1e-6);
        let frac = self.chart.anchor_frac.clamp(0.05, 0.95);
        let x_min = last_center - frac * span;
        ...
        ui.allocate_ui(egui::vec2(avail_w, candles_h), |ui| {
            Plot::new("candles_plot")
                ...
                .show(ui, |plot_ui| {
                    ...
                    // rulers: current candle center (doesn't move when TF changes)
                    let now_x = last_center;
        ...
        // ---- Volume panel (per-candle volume bars) ----
            ...
            Plot::new("volume_plot")
                ...
                .show(ui, |plot_ui| {
                    ...
                    for c in visible {
                        ...
                        plot_ui.line(Line::new(pts).color(Color32::LIGHT_GRAY));
                    }
                });
        });
        ...
        ui.allocate_ui(egui::vec2(avail_w, rsi_h), |ui| {
            ...
            for (idx_f, v) in rsi_all {
                ...
                if idx >= start_idx && idx < series_vec.len() {
                    let t = series_vec[idx].t as f64 + tf * 0.5;
            ...
            ui.columns(2, |cols| {
                cols[0].group(|ui| {
                    ...
                    if let Some(c) = series_vec.last() {
                        ...
                        ui.label(format!("Vol: {:.2}", c.volume));
                    }
                });
...
fn main() {
    ...
    // Task 1: dYdX testnet orderbook deltas
    rt.spawn(async move {
    ...
    rt.spawn(async move {
        ...
        // Subaccount used for feed subscription (moved only once)
        ...
        tokio::spawn(async move {
            while let Some(msg) = sub_feed.recv().await {
                let mut st = acct_tx_feed.borrow().clone();
                match msg {
                ...
                let _ = acct_tx_feed.send(st);
            }
        });

        // Command loop
        while let Some(cmd) = trade_rx.recv().await {
            let mut st = acct_tx.borrow().clone();
            ...
            // Derive a fresh subaccount for order building (no move issues)
            let order_sub = match account.subaccount(0) {
                Ok(s) => s,
                Err(e) => {
                    st.last_error = Some(format!("subaccount derive error: {e}"));
                    let _ = acct_tx.send(st);
                    continue;
                }
            };
            ...
            let (_id, order) = match OrderBuilder::new(market, order_sub)
                .market(side, size)
                .reduce_only(false)
                .price(100)
