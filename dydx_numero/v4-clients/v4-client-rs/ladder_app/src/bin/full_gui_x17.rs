...
//
// Build:
//    cargo run --release -p ladder_app --bin full_gui_x16
//
...
impl ComboApp {
    fn new(
        ...
        // row 3: bubble panel + receipts
        row_cfgs[3].span_mode = RowSpanMode::Split3;
        row_cfgs[3].height_factor = 1.4;
...
"#.to_string();
    ...
    fn adjust_tf_via_hotkey(&mut self, up: bool) {
        if TF_OPTS.is_empty() {
            return;
        }
        let mut idx = TF_OPTS
            .iter()
            .position(|(tf, _)| *tf == self.chart.tf_secs)
            .unwrap_or(0);
        if up {
            if idx + 1 < TF_OPTS.len() {
                idx += 1;
            }
        } else if idx > 0 {
            idx -= 1;
        }
        self.chart.tf_secs = TF_OPTS[idx].0;
    }

    fn send_manual_order(&mut self) {
        ...
        if let Ok(size_bd) = BigDecimal::from_str(&size_str) {
            ...
            self.push_receipt(TradeReceipt {
                ts: now_unix(),
                ticker,
                side: format!("{:?}", self.trade_side),
                kind: format!("{:?}", self.trade_kind),
                size: size_str,
                status: "submitted".to_string(),
                comment: "manual".to_string(),
            });
        } else {
            self.last_order_msg = "Invalid size for order".to_string();
        }
    }
    ...
    fn handle_hotkeys(&mut self, ctx: &egui::Context) {
        ...
        ctx.input(|i| {
            ...
            if i.key_pressed(egui::Key::Enter) {
                self.send_manual_order();
            }
            if i.key_pressed(egui::Key::F1) {
                self.show_hotkey_help = !self.show_hotkey_help;
            }
            if i.key_pressed(egui::Key::F2) {
                self.show_bubble_panel = !self.show_bubble_panel;
            }
            if i.key_pressed(egui::Key::F3) {
                self.script_auto_run = !self.script_auto_run;
            }
            if i.key_pressed(egui::Key::ArrowUp) {
                self.adjust_tf_via_hotkey(true);
            }
            if i.key_pressed(egui::Key::ArrowDown) {
                self.adjust_tf_via_hotkey(false);
            }
        });
    }
    ...
    fn ui_top_bar(&mut self, ui: &mut egui::Ui, snap_opt: Option<&Snapshot>) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.checkbox(&mut self.show_bubble_panel, "Bubble panel");
        });
        ...
        if let Some(snap) = snap_opt {
            ...
            ui.colored_label(
                ...
                format!(
                    "Mid: {:.2}  Spread: {:.4}  Imb: {:.2}",
                    bm.mid, bm.spread, bm.imbalance
                ),
            );
        }
    }

    fn ui_trading_panel(&mut self, ui: &mut egui::Ui, snap: Option<&Snapshot>) {
        let mid = snap.map(|s| s.last_mid).unwrap_or(0.0);
        ...
        ui.group(|ui| {
            ...
            ui.colored_label(
                ...
                "Requires DYDX_TESTNET_MNEMONIC + client/tests/testnet.toml",
            );
            ...
            ui.horizontal(|ui| {
                ...
                let buy_clicked = ui
                    .add_sized(
                        [60.0, 30.0],
                        egui::Button::new("BUY")
                            .fill(Color32::from_rgb(0, 200, 120)),
                    )
                    .clicked();
                let sell_clicked = ui
                    .add_sized(
                        ...
                        egui::Button::new("SELL")
                            .fill(Color32::from_rgb(230, 60, 100)),
                    )
                    .clicked();
                if buy_clicked {
                    self.trade_side = OrderSide::Buy;
            ...
            ui.horizontal(|ui| {
                ...
                let m_clicked = ui
                    .add_sized(
                        [80.0, 28.0],
                        egui::Button::new("Market")
                            .fill(Color32::from_rgb(90, 90, 190)),
                    )
                    .clicked();
                let l_clicked = ui
                    .add_sized(
                        [80.0, 28.0],
                        egui::Button::new("Limit-ish")
                            .fill(Color32::from_rgb(150, 90, 190)),
                    )
                    .clicked();
                if m_clicked {
            ...
            let notional = if mid > 0.0 {
                mid * self.trade_size_units * self.trade_leverage
            } else {
                0.0
            };
            ...
            ui.colored_label(
                ...
                format!(
                    "Sim balance: {:.2} USDC | PnL: {:.2} | Est. notional: {:.2} (mid {:.2})",
                    self.balance_usdc, self.balance_pnl, notional, mid
                ),
            );

            ui.separator();

            ui.label("Sim deposit / withdraw:");
            ui.horizontal(|ui| {
                ui.label("Amount:");
                ui.add(
                    egui::DragValue::new(&mut self.dw_amount)
                        .speed(10.0)
                ...
                if ui
                    .add_sized(
                        ...
                        egui::Button::new("Deposit")
                            .fill(Color32::from_rgb(60, 180, 120)),
                    )
                    .clicked()
                {
                    self.balance_usdc += self.dw_amount.abs();
                    self.push_receipt(TradeReceipt {
                        ts: now_unix(),
                        ticker: self.current_ticker.clone(),
                        side: "N/A".to_string(),
                        kind: "DepositSim".to_string(),
                        size: format!("{:.2}", self.dw_amount.abs()),
                        status: "ok".to_string(),
                ...
                if ui
                    .add_sized(
                        ...
                        egui::Button::new("Withdraw")
                            .fill(Color32::from_rgb(200, 90, 90)),
                    )
                    .clicked()
                {
                    let amt = self.dw_amount.abs();
                    if self.balance_usdc >= amt {
                        self.balance_usdc -= amt;
                        self.push_receipt(TradeReceipt {
                            ts: now_unix(),
                            ticker: self.current_ticker.clone(),
                            side: "N/A".to_string(),
                ...
                && self.bot_signal != "none"
            {
                ...
                if ui
                    .add_sized(
                        [ui.available_width() * 0.5, 26.0],
                        egui::Button::new("Use bot size"),
                    )
                    .clicked()
                {
            ...
            if ui
                .add_sized(
                    ...
                    egui::Button::new("SEND ORDER (Enter)")
                        .fill(Color32::from_rgb(60, 180, 250)),
                )
                .clicked()
            {
                self.send_manual_order();
                ...
                let plot_resp = Plot::new("candles_plot")
                    ...
                    .show(ui, |plot_ui| {
                        ...
                        for c in visible {
                            ...
                            let color_body = if up {
                                Color32::from_rgb(0, 255, 150)
                            } else {
                                Color32::from_rgb(255, 80, 140)
                            };
                            let color_wick = Color32::from_rgb(230, 230, 230);
                            ...
                            .into();
                            plot_ui.polygon(
                                Polygon::new(body_poly).fill_color(color_body),
                            );
                        }
                    });
        ...
        ui.group(|ui| {
            ...
            ui.colored_label(
                ...
                format!(
                    ...
                    bm.best_bid, bm.best_ask
                ),
            );
            ui.label(format!("Mid: {:.4}", bm.mid));
            ui.label(format!("Spread: {:.6}", bm.spread));
            ui.label(format!(
            ...
            ui.colored_label(
                ...
                format!("Imbalance (bid/ask): {:.2}", bm.imbalance),
            );

            ui.separator();
            ui.label("Use Rhai script to react to these metrics.");
            ui.label("Hotkey F2 toggles this panel.");
        });
    }

    fn ui_receipts_panel(&self, ui: &mut egui::Ui) {
        ui.group(|ui| {
            ui.heading("Trade receipts (UI-level)");
            egui::ScrollArea::vertical().show(ui, |ui| {
                egui::Grid::new("receipts_grid")
                    .striped(true)
                    .show(ui, |ui| {
                        ui.label("Time");
                        ui.label("Ticker");
                        ...
                        for r in self.receipts.iter().rev() {
                            ui.label(format_ts(self.time_mode, r.ts));
                            ui.label(&r.ticker);
                            ui.label(&r.side);
                            ui.label(&r.kind);
                            ui.label(&r.size);
    ...
    fn update(
        ...
        if self.show_hotkey_help {
            egui::Window::new("Hotkeys")
                ...
                .show(ctx, |ui| {
                    ui.label("B: side = BUY");
                    ui.label("S: side = SELL");
                    ui.label("Enter: send order");
                    ui.label("F1: toggle this help");
                    ui.label("F2: toggle bubble panel");
                    ui.label("F3: toggle script auto-run");
                    ui.label("Arrow Up/Down: change timeframe");
                });
        }
...
fn main() {
    ...
    if let Err(e) = eframe::run_native(
        "dYdX CSV Live + Replay + Script Bot (full_gui_x16)",
        native_options,
