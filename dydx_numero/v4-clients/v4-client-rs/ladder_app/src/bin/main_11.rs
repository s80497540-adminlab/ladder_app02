...
//
//   --- TP / SL controls (fake) ---
//   u = move Take Profit up   (price + 5)
//   j = move Take Profit down (price - 5)
//   i = move Stop Loss up     (price + 5)
//   k = move Stop Loss down   (price - 5)
//
//   q = quit
...
impl Widget for PositionOverlay {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let draw_line = |price: f64, color: Color, buf: &mut Buffer, area: Rect, row_min: i32, row_max: i32, map_price_to_row: &dyn Fn(f64) -> i32| {
        ...
        if event::poll(Duration::from_millis(1))? {
            if let Event::Key(k) = event::read()? {
                match k.code {
                    ...
                    // u/j => TP up/down
                    KeyCode::Char('u') => {
                        trading.bump_tp(last_price, 5.0);
                    }
                    KeyCode::Char('j') => {
                        trading.bump_tp(last_price, -5.0);
                    }
                    ...
                    // i/k => SL up/down
                    KeyCode::Char('i') => {
                        trading.bump_sl(last_price, 5.0);
                    }
                    KeyCode::Char('k') => {
                        trading.bump_sl(last_price, -5.0);
                    }
        ...
        terminal.draw(|f| {
            ...
            let tabs_line = {
                ...
                Paragraph::new(Line::from(vec![
                    ...
                    Span::raw("   ||  Trade: b=LONG s=SHORT f=FLAT  9/0=margin -/=pos [/]=lev u/j=TP i/k=SL"),
                    Span::raw(" | mode: "),
...
fn trading_widget(trading: &TradingState, mark_price: f64) -> Paragraph<'static> {
    ...
    let hint_line = "Enter=open/close  |  b=LONG s=SHORT f=FLAT  9/0=margin  -/=pos  [/]=lev  u/j=TP  i/k=SL";
