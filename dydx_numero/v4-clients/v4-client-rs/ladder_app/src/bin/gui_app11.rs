...
//  - Current candle locked in place across TF changes (anchored to "now")
...
struct MyApp {
    selected_tab: Tab,
    last_price: f64,
    last_ts: u64, // anchor "now" so right edge stays locked across TF changes
...
impl MyApp {
    ...
    fn tick(&mut self) {
        ...
        // synthetic volume per tick for now (1.0). Later: plug real trade size.
        let vol_tick = 1.0;
    ...
    fn ui_top_bar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            // Ticker selection combo (popup style, inline)
    ...
    fn ui_candles(&mut self, ui: &mut egui::Ui) {
        ...
        // rebalanced layout: price, volume, RSI, bottom info
        ...
        ui.allocate_ui(egui::vec2(avail_w, candles_h), |ui| {
            Plot::new("candles_plot")
                ...
                .show(ui, |plot_ui| {
                    ...
                    let x_max = (self.last_ts as f64) + self.chart.x_pan_secs;
                    ...
                    // rulers: current price/time, anchored to last_ts (so it doesn't move with TF)
                    let now_x = self.last_ts as f64;
                    let now_px = visible.last().unwrap().close;
...
fn main() {
    ...
    rt.spawn(async move {
        ...
        // Subaccount used for feed subscription (moved once here, never reused)
        ...
        while let Some(cmd) = trade_rx.recv().await {
            ...
            // Derive a fresh Subaccount for order building to avoid "use of moved value"
