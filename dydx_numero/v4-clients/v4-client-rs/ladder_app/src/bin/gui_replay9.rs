...
// - Supports ETH-USD, BTC-USD, SOL-USD (SOL will show even if there's no data yet)
//
...
fn price_to_key(price: f64) -> PriceKey {
    // store prices with 1e-4 precision as integers
...
impl ReplayApp {
    fn new(base_dir: &str) -> Self {
        // always show these three in the menu;
        // SOL-USD may not have data yet, but that's fine.
        let tickers = vec![
            "ETH-USD".to_string(),
            "BTC-USD".to_string(),
            "SOL-USD".to_string(),
    ...
    fn ui_top_bar(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(td) = self.current_ticker_data() {
            ...
            ui.label("No CSV data available for this ticker.");
        }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            if let Some(snap) = &snapshot {
                ...
                ui.heading("No data loaded for this ticker.");
