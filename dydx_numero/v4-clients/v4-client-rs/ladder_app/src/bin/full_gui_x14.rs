...
//      * Trading panel: market/“limit” (currently all build as market
//        with a price guard), size, leverage field, bot suggestions.
//
//  Requirements:
...
impl ComboApp {
    fn new(
        ...
        // simple oneliner: we keep it mostly stock; user writes logic
        engine.set_max_expr_depths(64, 64);

        let mut scope = Scope::new();

        // default script: simple bubble detector based on depth imbalance
        let default_script = r#"
// Rhai bot script.
// Inputs:
//   best_bid, best_ask, mid, spread
//   bid_liquidity_near, ask_liquidity_near
//   tf_secs, history_candles
//
// Outputs (read back by Rust):
...
"#.to_string();
    ...
    fn feed_scope_from_snapshot(&mut self, snap: &Snapshot) {
        ...
        // liquidity near top (within first ~10 levels)
        let mut bid_liq = 0.0;
        ...
        ui.group(|ui| {
            ...
            ui.horizontal(|ui| {
                if ui.button("Run now").clicked() {
                    // we actually run from update() when we have a snap
                    // so here we just mark timestamp so that the next
                    // update will call run_script with the snapshot.
    ...
    fn ui_grid(&mut self, ui: &mut egui::Ui, snap_opt: Option<&Snapshot>) {
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                for row in 0..GRID_ROWS {
                    let cfg = self.row_cfgs[row].clone();
...
impl eframe::App for ComboApp {
    fn update(
        ...
        // periodic reload from daemon
        let now = now_unix();
        if now.saturating_sub(self.last_reload_ts)
            as f64
            >= self.reload_secs
        {
            self.reload_current_ticker();
        ...
        if self.script_auto_run {
            if let Some(ref snap) = snap_opt {
                // throttle a bit: at most once per 200ms
...
async fn run_trader(mut rx: mpsc::Receiver<TradeCmd>) {
    ...
    while let Some(cmd) = rx.recv().await {
        ...
        // NOTE:
        //   For now we still use .market() for both "Market" and "Limit"
        //   kinds, and interpret "limit_price" as a future guard that you
        //   can wire in yourself. True limit orders would require calling
        //   the proper limit method on OrderBuilder.
        ...
        // trivial price guard usage (this is effectively a slippage cap)
        if limit_price > 0.0 {
            // this is a placeholder: OrderBuilder::price expects a BigInt
            // type in the real client; we used 100 earlier as a dummy.
            // We'll keep that style for now and let you refine it.
            let _ = builder.price(100);
        }
...
fn main() {
    ...
    // runtime will drop here, which is fine because UI is exiting
}
