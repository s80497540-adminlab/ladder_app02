...
use egui_plot::{
    GridMark, HLine, Line, Plot, PlotBounds, PlotPoints, Polygon, VLine,
};
...
// simple in-memory orderbook (f64)
#[derive(Default, Clone, Debug)]
struct LiveBook {
    pub bids: BTreeMap<f64, f64>, // price -> size
...
impl LiveBook {
    ...
    fn best_bid_ask(&self) -> (Option<(f64, f64)>, Option<(f64, f64)>) {
        let bid = self.bids.iter().next_back().map(|(p, s)| (*p, *s));
        let ask = self.asks.iter().next().map(|(p, s)| (*p, *s));
        (bid, ask)
    }
}
...
impl ReplayApp {
        ...
        ui.allocate_ui(egui::vec2(avail_w, avail_h), |ui| {
            ui.horizontal(|ui| {
                ...
                ui.allocate_ui(egui::vec2(left_w, avail_h), |ui| {
                    ...
                    let mut cum = 0.0;
                    for (p, s) in self.book.bids.iter().rev() {
                        cum += *s;
                        bid_points.push((*p, cum));
                    }

                    cum = 0.0;
                    for (p, s) in self.book.asks.iter() {
                        cum += *s;
                    ...
                    ui.columns(2, |cols| {
                        ...
                        egui::Grid::new("bids_grid_replay")
                            ...
                            .show(&mut cols[0], |ui| {
                                ...
                                ui.label("Size");
                                ui.end_row();
                                for (p, s) in self.book.bids.iter().rev().take(15) {
                        ...
                        egui::Grid::new("asks_grid_replay")
                            ...
                            .show(&mut cols[1], |ui| {
                                ...
                                ui.label("Size");
                                ui.end_row();
                                for (p, s) in self.book.asks.iter().take(15) {
                                    ui.label(format!("{:>8.2}", p));
                                    ui.label(format!("{:>6.4}", s));
                                    ui.end_row();
                                }
    ...
    fn ui_candles(&mut self, ui: &mut egui::Ui) {
        ...
        // candles plot (same style as live, filled bodies, tuned wheel)
