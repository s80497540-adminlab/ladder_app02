...
impl ComboApp {
    ...
    fn tick_live(&mut self) {
        ...
        if let Some(mid) = self.live_book.mid() {
            let vol = 0.0; // placeholder volume
    ...
    fn ensure_replay_ts_in_range(&mut self) {
        // avoid borrowing self while mutating replay_ts
...
fn main() {
    ...
    // ticker_tx is ready if we later want UI-driven live ticker switching
    let _ticker_tx = ticker_tx;
