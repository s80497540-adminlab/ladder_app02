// ladder_app/src/bin/gui_replay4.rs
//
// Offline REPLAY app.
...
//   - Candles + volume + RSI (filled bodies)
...
//   2. Then run this:
//
//      cargo run -p ladder_app --bin gui_replay4
//
...
impl ReplayApp {
    ...
    fn switch_tf(&mut self, new_tf: u64) {
        ...
        self.candles_bounds = None; // recenter on new TF
    }
    ...
    fn seek_to(&mut self, ts_target: u64) {
        ...
        self.sim_ts = target;
        self.candles_bounds = None; // recenter after jump
    }
    ...
    fn step_sim(&mut self) {
        ...
        self.trading
            .check_liquidation(self.last_price, self.sim_ts);
    }
    ...
    fn ui_candles(&mut self, ui: &mut egui::Ui) {
        ...
        // candles plot (filled bodies, tuned wheel, time axis formatter, CENTER current candle by default)
        ui.allocate_ui(egui::vec2(avail_w, candles_h), |ui| {
            ...
            Plot::new("candles_plot_replay")
                ...
                .show(ui, |plot_ui| {
                    ...
                    // First time: center current candle in the middle of the chart
                    ...
                    // After first frame: respect user scroll direction (X vs Y only) using raw scroll delta
                        ...
                        ctx.input(|i| {
                            if i.raw_scroll_delta.y != 0.0 {
                                if i.modifiers.shift {
                                    restore_x = true; // Shift+wheel -> Y-only zoom
                                } else {
                                    restore_y = true; // wheel -> X-only zoom
                                }
                            }
                        });
