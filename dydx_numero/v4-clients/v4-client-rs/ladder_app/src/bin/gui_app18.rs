// ladder_app/src/bin/gui_app17.rs
//
...
//   cargo run -p ladder_app --bin gui_app17
...
impl MyApp {
    ...
    fn switch_tf(&mut self, new_tf: u64) {
        ...
        // with our drawing model x_max = anchor + x_pan_secs
        // and x_min = x_max - span
    ...
    fn tick(&mut self) {
        ...
        // simple dummy volume for now (can wire to real trades later)
        let volume = 1.0;
            ...
            ui.horizontal(|ui| {
                if ui.button("Market BUY (real)").clicked() {
                    ...
                    if let Ok(size) = BigDecimal::from_str(&s_str) {
                        let _ = self.trade_tx.try_send(TradeCmd::MarketBuy { ticker: t, size });
                    }
                }
    ...
    fn ui_candles(&mut self, ui: &mut egui::Ui) {
        ...
        ui.allocate_ui(egui::vec2(avail_w, candles_h), |ui| {
            ...
            Plot::new("candles_plot")
                ...
                .include_y(y_min)
                .include_y(y_max)
                .x_axis_formatter(move |x, _range, _transform| format_ts_common(mode, x as u64))
                .show(ui, |plot_ui| {
...
fn main() {
    ...
    rt.spawn(async move {
        ...
        // we need two copies of sub: one for feed, one for orders
        let sub_for_orders = sub;
        let sub_for_feed = sub_for_orders.clone();
        ...
        while let Some(cmd) = trade_rx.recv().await {
            ...
            let (_id, order) = match OrderBuilder::new(market, sub_for_orders.clone())
                .market(side, size)
                .reduce_only(false)
                .price(100)
