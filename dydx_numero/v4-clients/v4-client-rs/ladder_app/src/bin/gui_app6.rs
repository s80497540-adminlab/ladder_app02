...
//  - Position max AUTO-derives from margin*lev/price
//  - NEW: Liquidation mechanism (maintenance margin)
...
struct TradingState {
    ...
    // --- liquidation ---
    maint_rate: f64,            // maintenance margin rate, e.g. 0.005 = 0.5%
    last_liq_price: Option<f64>,
    last_liq_time: Option<u64>, // unix seconds
...
impl TradingState {
    fn new() -> Self {
        Self {
            ...
            maint_rate: 0.005, // 0.5% maint margin (fake, tweak later)
            last_liq_price: None,
            last_liq_time: None,
    ...
    /// Max position units you can hold given margin/leverage/mark.
    ...
    fn open_at(&mut self, mark: f64) {
        ...
        // If user didn't set size, default to max size.
        ...
        self.liquidated_flag = false; // clear banner on fresh open
    }
    ...
    fn liquidate_at(&mut self, mark: f64, ts: u64) {
        ...
        // realize PnL at liquidation price
        let upnl = self.unrealized_pnl(mark);
        self.realized_pnl += upnl;
...
impl MyApp {
    ...
    fn tick(&mut self) {
        ...
        // TP/SL first, then liquidation check
