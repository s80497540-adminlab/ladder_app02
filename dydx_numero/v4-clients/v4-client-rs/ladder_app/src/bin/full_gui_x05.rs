// ladder_app/src/bin/full_gui_x05.rs
//
...
//   - Rhai script engine for appearance/layout tuning
//   - Custom theme mode with per-color RGB from script
//
// Run (with daemon already running):
//   cargo run -p ladder_app --bin full_gui_x05
//
...
impl Theme {
    fn from_kind(kind: ThemeKind) -> Self {
        match kind {
            ...
            ThemeKind::Custom => Self {
                // will be overridden by ComboApp.custom_theme
                bg: Color32::from_rgb(15, 15, 20),
                text: Color32::from_rgb(230, 230, 240),
                candle_up: Color32::from_rgb(80, 200, 120),
                candle_down: Color32::from_rgb(240, 80, 80),
...
impl ComboApp {
    fn new(base_dir: String) -> Self {
        ...
        let base_theme = Theme::from_kind(ThemeKind::Dark);

        let engine = Engine::new();
        let scope: Scope<'static> = Scope::new();
        let script = String::from(
            "// Rhai script example:\n\
             // theme           = \"Matrix\";    // or \"Dark\", \"Light\", \"Ocean\", \"Fire\", \"Custom\"\n\
             // ticker          = \"ETH-USD\";   // or BTC-USD / SOL-USD\n\
             // mode            = \"Live\";      // or \"Replay\"\n\
             // replay_tab      = \"Candles\";   // or \"Orderbook\"\n\
             // time_mode       = \"Local\";     // or \"Unix\"\n\
             // tf              = 60;           // timeframe in seconds\n\
             // history         = 400;          // candles shown\n\
             // show_volume     = false;\n\
             // show_depth      = true;\n\
             // x_zoom          = 2.0;\n\
             // x_pan_secs      = -300.0;\n\
             ...
             // y_min           = 2000.0;\n\
             // y_max           = 2500.0;\n\
             // depth_height_frac = 0.40;       // 0.15 - 0.7\n\
             // ladders_rows      = 40;         // 5 - 200\n\
             // trades_rows       = 150;        // 5 - 500\n\
             // reload_secs     = 0.5;\n\
             //\n\
             // // Custom colors (only used when theme = \"Custom\"):\n\
             // bg_r = 5;  bg_g = 10;  bg_b = 20;\n\
             // text_r = 230; text_g = 245; text_b = 255;\n\
             // candle_up_r = 0; candle_up_g = 220; candle_up_b = 140;\n\
             // candle_down_r = 255; candle_down_g = 100; candle_down_b = 100;\n\
             // volume_r = 90; volume_g = 150; volume_b = 240;\n\
             // bid_line_r = 0; bid_line_g = 255; bid_line_b = 120;\n\
             // ask_line_r = 255; ask_line_g = 190; ask_line_b = 80;\n",
        );
    ...
    fn switch_ticker(&mut self, new_ticker: &str) {
        if !self.tickers.iter().any(|t| t == new_ticker) {
            return;
        }
    ...
    fn run_script(&mut self) {
        ...
        self.scope
            .set_value("x_zoom", self.chart.x_zoom as f64);
        self.scope
            .set_value("x_pan_secs", self.chart.x_pan_secs);
        self.scope.set_value(
            "depth_height_frac",
            self.layout.depth_height_frac as f64,
        );
        self.scope
            .set_value("ladders_rows", self.layout.ladders_rows as i64);
        self.scope
            .set_value("trades_rows", self.layout.trades_rows as i64);
        ...
        self.scope
            .set_value("theme", theme_str.to_string());
        self.scope
        ...
        self.scope.set_value("mode", mode_str.to_string());

        let time_mode_str = match self.time_mode {
            TimeDisplayMode::Local => "Local",
            TimeDisplayMode::Unix => "Unix",
        };
        self.scope
            .set_value("time_mode", time_mode_str.to_string());

        let replay_tab_str = match self.replay_tab {
            ReplayTab::Orderbook => "Orderbook",
            ReplayTab::Candles => "Candles",
        };
        self.scope
            .set_value("replay_tab", replay_tab_str.to_string());

        // seed color channels from current theme
        let th = &self.theme;
        self.scope
            .set_value("bg_r", th.bg.r() as i64);
        self.scope
            .set_value("bg_g", th.bg.g() as i64);
        self.scope
            .set_value("bg_b", th.bg.b() as i64);

        self.scope
            .set_value("text_r", th.text.r() as i64);
        self.scope
            .set_value("text_g", th.text.g() as i64);
        self.scope
            .set_value("text_b", th.text.b() as i64);

        self.scope
            .set_value("candle_up_r", th.candle_up.r() as i64);
        self.scope
            .set_value("candle_up_g", th.candle_up.g() as i64);
        self.scope
            .set_value("candle_up_b", th.candle_up.b() as i64);

        self.scope
            .set_value("candle_down_r", th.candle_down.r() as i64);
        self.scope
            .set_value("candle_down_g", th.candle_down.g() as i64);
        self.scope
            .set_value("candle_down_b", th.candle_down.b() as i64);

        self.scope
            .set_value("volume_r", th.volume.r() as i64);
        self.scope
            .set_value("volume_g", th.volume.g() as i64);
        self.scope
            .set_value("volume_b", th.volume.b() as i64);

        self.scope
            .set_value("bid_line_r", th.bid_line.r() as i64);
        self.scope
            .set_value("bid_line_g", th.bid_line.g() as i64);
        self.scope
            .set_value("bid_line_b", th.bid_line.b() as i64);

        self.scope
            .set_value("ask_line_r", th.ask_line.r() as i64);
        self.scope
            .set_value("ask_line_g", th.ask_line.g() as i64);
        self.scope
            .set_value("ask_line_b", th.ask_line.b() as i64);

        self.script_status.clear();

        match self
            .engine
            .eval_with_scope::<Dynamic>(&mut self.scope, &self.script)
        {
            Ok(_) => {
                // Pull values back out if present

                // tf
                if let Some(tf_val) = self.scope.get_value::<i64>("tf") {
                    if tf_val > 0 {
                        let tf_u = tf_val as u64;
                        if TF_LIST.iter().any(|(tf, _)| *tf == tf_u) {
                            self.chart.selected_tf = tf_u;
                        }
                    }
                }

                // history
                if let Some(history) = self.scope.get_value::<i64>("history") {
                    if history > 0 {
                        self.chart.show_candles =
                            history.clamp(10, 5000) as usize;
                    }
                }
                ...
                // reload interval
                if let Some(reload_secs) =
                    self.scope.get_value::<f64>("reload_secs")
                {
                    if reload_secs > 0.05 {
                        self.reload_secs =
                            reload_secs.clamp(0.05, 10.0) as f32;
                    }
                }

                // x zoom / pan
                if let Some(x_zoom) = self.scope.get_value::<f64>("x_zoom") {
                    self.chart.x_zoom = x_zoom.clamp(0.25, 8.0);
                }
                if let Some(x_pan) = self.scope.get_value::<f64>("x_pan_secs")
                {
                    self.chart.x_pan_secs = x_pan;
                }

                // depth panel height
                if let Some(df) =
                    self.scope.get_value::<f64>("depth_height_frac")
                {
                    let df = df.clamp(0.15, 0.7);
                    self.layout.depth_height_frac = df as f32;
                }

                // rows
                if let Some(rows) =
                    self.scope.get_value::<i64>("ladders_rows")
                {
                    let r = rows.clamp(5, 200);
                    self.layout.ladders_rows = r as usize;
                }
                if let Some(rows) =
                    self.scope.get_value::<i64>("trades_rows")
                {
                    let r = rows.clamp(5, 500);
                    self.layout.trades_rows = r as usize;
                }

                // theme from script
                if let Some(theme) =
                    self.scope.get_value::<String>("theme")
                {
                    let lower = theme.to_lowercase();
                    self.theme_kind = if lower.contains("dark") {
                        ThemeKind::Dark
                ...
                // ticker from script
                if let Some(ticker) =
                    self.scope.get_value::<String>("ticker")
                {
                    self.switch_ticker(&ticker);
                }

                // mode from script
                if let Some(mode_s) =
                    self.scope.get_value::<String>("mode")
                {
                    let lower = mode_s.to_lowercase();
                    self.mode = if lower.contains("replay") {
                        Mode::Replay
                    } else {
                        Mode::Live
                    };
                ...
                // time_mode from script
                if let Some(tm_s) =
                    self.scope.get_value::<String>("time_mode")
                {
                    let lower = tm_s.to_lowercase();
                    self.time_mode = if lower.contains("unix") {
                ...
                // replay_tab from script
                if let Some(rt_s) =
                    self.scope.get_value::<String>("replay_tab")
                {
                    let lower = rt_s.to_lowercase();
                    self.replay_tab = if lower.contains("order") {
                        ReplayTab::Orderbook
                    } else {
                        ReplayTab::Candles
                    };
                ...
                // --- COLOR OVERRIDES (populate custom_theme) ---

                let mut custom = self.custom_theme.clone();
                ...
                // bg
                let mut bg_r = custom.bg.r();
                let mut bg_g = custom.bg.g();
                let mut bg_b = custom.bg.b();
                if let Some(v) = self.scope.get_value::<i64>("bg_r") {
                    bg_r = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("bg_g") {
                    bg_g = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("bg_b") {
                    bg_b = clamp_u8(v);
                }
                custom.bg = Color32::from_rgb(bg_r, bg_g, bg_b);

                // text
                let mut tx_r = custom.text.r();
                let mut tx_g = custom.text.g();
                let mut tx_b = custom.text.b();
                if let Some(v) = self.scope.get_value::<i64>("text_r") {
                    tx_r = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("text_g") {
                    tx_g = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("text_b") {
                    tx_b = clamp_u8(v);
                }
                custom.text = Color32::from_rgb(tx_r, tx_g, tx_b);

                // candle_up
                let mut cu_r = custom.candle_up.r();
                let mut cu_g = custom.candle_up.g();
                let mut cu_b = custom.candle_up.b();
                if let Some(v) = self.scope.get_value::<i64>("candle_up_r") {
                    cu_r = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("candle_up_g") {
                    cu_g = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("candle_up_b") {
                    cu_b = clamp_u8(v);
                }
                custom.candle_up = Color32::from_rgb(cu_r, cu_g, cu_b);

                // candle_down
                let mut cd_r = custom.candle_down.r();
                let mut cd_g = custom.candle_down.g();
                let mut cd_b = custom.candle_down.b();
                if let Some(v) = self.scope.get_value::<i64>("candle_down_r") {
                    cd_r = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("candle_down_g") {
                    cd_g = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("candle_down_b") {
                    cd_b = clamp_u8(v);
                }
                custom.candle_down = Color32::from_rgb(cd_r, cd_g, cd_b);

                // volume
                let mut vol_r = custom.volume.r();
                let mut vol_g = custom.volume.g();
                let mut vol_b = custom.volume.b();
                if let Some(v) = self.scope.get_value::<i64>("volume_r") {
                    vol_r = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("volume_g") {
                    vol_g = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("volume_b") {
                    vol_b = clamp_u8(v);
                }
                custom.volume = Color32::from_rgb(vol_r, vol_g, vol_b);

                // bid_line
                let mut bl_r = custom.bid_line.r();
                let mut bl_g = custom.bid_line.g();
                let mut bl_b = custom.bid_line.b();
                if let Some(v) = self.scope.get_value::<i64>("bid_line_r") {
                    bl_r = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("bid_line_g") {
                    bl_g = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("bid_line_b") {
                    bl_b = clamp_u8(v);
                }
                custom.bid_line = Color32::from_rgb(bl_r, bl_g, bl_b);

                // ask_line
                let mut al_r = custom.ask_line.r();
                let mut al_g = custom.ask_line.g();
                let mut al_b = custom.ask_line.b();
                if let Some(v) = self.scope.get_value::<i64>("ask_line_r") {
                    al_r = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("ask_line_g") {
                    al_g = clamp_u8(v);
                }
                if let Some(v) = self.scope.get_value::<i64>("ask_line_b") {
                    al_b = clamp_u8(v);
                }
                custom.ask_line = Color32::from_rgb(al_r, al_g, al_b);

                self.custom_theme = custom;

                // if we're currently on Custom, apply immediately
                if matches!(self.theme_kind, ThemeKind::Custom) {
                    self.theme = self.custom_theme.clone();
                }

                self.script_status = "Script applied".to_string();
            }
            Err(e) => {
                self.script_status = format!("Script error: {e}");
            }
        }
    }
    ...
    fn ui_top_bar(&mut self, ui: &mut egui::Ui) {
        ...
        self.theme = match self.theme_kind {
            ThemeKind::Custom => self.custom_theme.clone(),
            _ => Theme::from_kind(self.theme_kind),
        };
        ...
        ui.horizontal(|ui| {
            ui.label("Depth panel height:");
            ui.add(
                egui::Slider::new(
                    &mut self.layout.depth_height_frac,
                    0.15..=0.7,
                )
                .text("depth %"),
            );
        ...
        // Script engine UI
        egui::CollapsingHeader::new("Script engine (Rhai)")
            .default_open(false)
            .show(ui, |ui| {
                ui.label(
                    RichText::new(
                        "You can tweak layout/appearance live.\n\
                         Available vars:\n\
                         - theme: string (\"Dark\", \"Light\", \"Ocean\", \"Fire\", \"Matrix\", \"Custom\")\n\
                         - ticker: \"ETH-USD\", \"BTC-USD\", \"SOL-USD\"\n\
                         - mode: \"Live\" or \"Replay\"\n\
                         - replay_tab: \"Orderbook\" or \"Candles\"\n\
                         - time_mode: \"Local\" or \"Unix\"\n\
                         - tf: integer timeframe in seconds (must match TF_LIST)\n\
                         - history: candles shown\n\
                         - auto_y: bool\n\
                         - y_min, y_max: f64\n\
                         - show_depth, show_ladders, show_trades, show_volume: bool\n\
                         - x_zoom: f64 (0.25..=8.0)\n\
                         - x_pan_secs: f64\n\
                         - depth_height_frac: f64 (0.15..=0.7)\n\
                         - ladders_rows: i64 (5..=200)\n\
                         - trades_rows: i64 (5..=500)\n\
                         - reload_secs: f64 (0.05..=10.0)\n\
                         - For theme = \"Custom\", you can also set RGB channels:\n\
                         - bg_r,g,b; text_r,g,b; candle_up_r,g,b; candle_down_r,g,b;\n\
                           volume_r,g,b; bid_line_r,g,b; ask_line_r,g,b (0..255).\n",
                    )
                    .color(self.theme.text),
                );
                ui.add(
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.button("Clear").clicked() {
                        self.script.clear();
                        self.script_status = "Script cleared".to_string();
                    }
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                let depth_h = if self.layout.show_depth
                    || self.layout.show_ladders
                    || self.layout.show_trades
                {
...
fn main() {
    ...
    if let Err(e) = eframe::run_native(
        "dYdX CSV Viewer (daemon-powered) + Rhai + Custom Theme",
        options,
