...
// dYdX client pieces – using the `dydx` crate that already exists in your workspace.
...
fn main() {
    // IMPORTANT: this is relative to the *workspace* root when you run `cargo run -p ladder_app02`
    let base_dir = PathBuf::from("ladder_app/data");
    let tickers = vec![
        "ETH-USD".to_string(),
        "BTC-USD".to_string(),
        "SOL-USD".to_string(),
...
"#;
    ...
    println!("Expected CSV dir (workspace-relative): {}", base_dir.display());
