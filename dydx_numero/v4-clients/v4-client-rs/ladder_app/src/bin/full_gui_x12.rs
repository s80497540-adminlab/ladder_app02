...
// shows per-ticker orderbook depth + ladders + trades + candles + volume.
//
// Layout:
//   - Central area is a scrollable 3x6 grid (18 cells).
//   - Each cell is a framed panel with a dropdown
//     to choose what it displays:
//       * Summary
//       * Depth
//       * Ladders + Trades
//       * Candles + Volume
...
//
// Data:
//   - Assumes a separate daemon (data_daemon02) is running and writing:
//       data/orderbook_{TICKER}.csv: ts,ticker,kind,side,price,size
//       data/trades_{TICKER}.csv:    ts,ticker,source,side,size_str
//   - GUI NEVER talks directly to dYdX for feeds; it only consumes CSVs.
//
// Bot / Script:
//   - Rhai script gets a ctx map with mid, best_bid/ask, bubble scores, last_vol.
//   - Script can control:
...
//
// Trading panel:
//   - Purely local: when you click "Record suggestion", it appends to
//     data/manual_trades.csv with timestamp, ticker, side, kind, size, leverage,
//     limit_price (if any), and a note.
//   - You can use it as a planner / confirm step for manual execution.
//
// Build & run:
...
// ---------- trading panel (local suggestions) ----------
...
fn append_manual_trade_suggestion(
    base_dir: &Path,
    ts: u64,
    ticker: &str,
    side: TradeSide,
    ...
    let path = base_dir.join("manual_trades.csv");
...
impl PanelKind {
    fn label(self) -> &'static str {
        match self {
            ...
            PanelKind::Depth => "Depth",
            PanelKind::LaddersTrades => "Ladders + Trades",
            PanelKind::Candles => "Candles + Volume",
            PanelKind::Script => "Script Engine",
            PanelKind::BotStatus => "Bot Status",
            PanelKind::Trading => "Trading Panel",
...
fn default_layout_slots() -> Vec<PanelKind> {
    // 3x6 grid -> 18 slots
    vec![
        // row 1
        PanelKind::Summary,
        PanelKind::Candles,      // chart visible immediately
        PanelKind::Depth,        // depthbook
        // row 2
        PanelKind::LaddersTrades,
        PanelKind::BotStatus,
        PanelKind::Trading,
...
impl ComboApp {
    fn new(base_dir: PathBuf) -> Self {
        ...
        let mut app = Self {
            ...
            layout_slots: default_layout_slots(),
            trade_side: TradeSide::Buy,
            trade_kind: TradeKind::Market,
            trade_size: 0.01,
            trade_leverage: 5.0,
            trade_limit_price: 0.0,
            trade_note: String::new(),
            trade_last_msg: String::new(),
        };
    ...
    fn ui_script_panel(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            if ui.button("Run now").clicked() {
                if let Some(snap) = self.current_snap().cloned() {
    ...
    fn ui_summary_panel(&self, ui: &mut egui::Ui) {
        ...
        if let Some(td) = self.current_ticker_data() {
            ...
            ui.label("No data for this ticker.");
        }
        ...
        if let Some(snap) = self.current_snap() {
            ...
            ui.label(format!(
                ...
                bubbles.best_bid, bubbles.best_ask
            ));
            ui.label(format!(
                "Bid wall: price={:.4} size={:.4} score={:.2}",
                bubbles.bid_wall_price, bubbles.bid_wall_size, bubbles.bid_wall_score
            ));
            ui.label(format!(
                "Ask wall: price={:.4} size={:.4} score={:.2}",
                bubbles.ask_wall_price, bubbles.ask_wall_size, bubbles.ask_wall_score
            ));
        }
    }
    ...
    fn ui_bot_status_panel(&self, ui: &mut egui::Ui) {
        ui.label(format!(
            "Bot signal: {} (size {:.6})",
            self.bot_signal, self.bot_size
        ));
        ...
        egui::ScrollArea::vertical()
            .max_height(120.0)
            .show(ui, |ui| {
                ui.label(&self.bot_comment);
            });
    }

    // ---------- trading panel (local suggestions only) ----------

    fn ui_trading_panel(&mut self, ui: &mut egui::Ui) {
        ui.label("Local Trade Suggestion Panel (no direct dYdX orders)");

        ui.horizontal(|ui| {
            ui.label("Side:");
            for side in [TradeSide::Buy, TradeSide::Sell] {
                ui.selectable_value(&mut self.trade_side, side, side.label());
            }
        });

        ui.horizontal(|ui| {
            ui.label("Type:");
            for kind in [TradeKind::Market, TradeKind::Limit] {
                ui.selectable_value(&mut self.trade_kind, kind, kind.label());
            }
        });
        ...
        ui.horizontal(|ui| {
            ui.label("Note:");
            ui.text_edit_singleline(&mut self.trade_note);
        });

        let eff_size = self.trade_size * self.trade_leverage.max(0.0);
        ui.label(format!(
            "Effective notional (size * leverage): {:.8} (units * x)",
            eff_size
        ));

        if ui.button("Record suggestion → data/manual_trades.csv").clicked() {
            let ts = match self.mode {
                Mode::Live => self.live_ts,
                Mode::Replay => self.replay_ts,
            };
            let limit_p = if matches!(self.trade_kind, TradeKind::Limit) {
                Some(self.trade_limit_price.max(0.0))
            } else {
                None
            };
            append_manual_trade_suggestion(
                &self.base_dir,
                ts,
                &self.current_ticker,
                self.trade_side,
            ...
            self.trade_last_msg = format!(
                "Recorded {} {} {} (kind={}, lev={:.2}, limit={:?})",
                self.current_ticker,
                self.trade_side.label(),
                self.trade_size,
                self.trade_kind.label(),
                self.trade_leverage,
                limit_p
            );
        }

        if !self.trade_last_msg.is_empty() {
            ui.separator();
            ui.label(&self.trade_last_msg);
        }
    }

    // ---------- depth / ladders / trades / candles ----------

    fn ui_depth_plot(&self, ui: &mut egui::Ui, snap: &Snapshot, height: f32) {
        if !self.show_depth {
            ui.label("Depth hidden by script / settings.");
            return;
        }
    ...
    fn ui_ladders_and_trades(&self, ui: &mut egui::Ui, snap: &Snapshot) {
        if !self.show_ladders && !self.show_trades {
            ui.label("Ladders & trades hidden by script / settings.");
            return;
        }
    ...
    fn ui_candles_and_volume(
        ...
        if self.show_volume {
            ...
            ui.label("Volume hidden by script / settings.");
        }
    }

    // ---------- per-cell panel container ----------

    fn ui_panel_container(&mut self, ui: &mut egui::Ui, idx: usize) {
        let kind = self
            .layout_slots
            .get(idx)
            .cloned()
            .unwrap_or(PanelKind::Empty);
        let mut chosen_kind = kind;

        egui::Frame::group(ui.style()).show(ui, |ui| {
            ui.horizontal(|ui| {
                ui.label(format!("Cell {}", idx + 1));
                egui::ComboBox::from_id_source(format!("panel_kind_{idx}"))
                    .selected_text(chosen_kind.label())
                    .show_ui(ui, |ui| {
            ...
            self.layout_slots[idx] = chosen_kind;

            ui.separator();

            match chosen_kind {
                PanelKind::Summary => self.ui_summary_panel(ui),
                PanelKind::Depth => {
                    if let Some(snap) = self.current_snap() {
                        self.ui_depth_plot(ui, snap, ui.available_height());
                    } else {
                        ui.label("No snapshot for this mode/ticker.");
                    }
                }
                PanelKind::LaddersTrades => {
                    if let Some(snap) = self.current_snap() {
                ...
                PanelKind::Candles => {
                    if let Some(snap) = self.current_snap().cloned() {
                        ...
                        self.ui_candles_and_volume(ui, &snap, is_live);
                    } else {
                        ui.label("No snapshot for this mode/ticker.");
                    }
                }
                PanelKind::Script => self.ui_script_panel(ui),
                PanelKind::BotStatus => self.ui_bot_status_panel(ui),
                PanelKind::Trading => self.ui_trading_panel(ui),
                PanelKind::Empty => {
                    ui.label("Empty cell. Use the dropdown above to assign content.");
                }
            }
        });
    }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                for row in 0..6 {
                    ui.push_id(format!("row_{row}"), |ui| {
                        ui.columns(3, |cols| {
                            for col in 0..3 {
                                let idx = row * 3 + col;
                                cols[col].vertical(|cell_ui| {
                                    cell_ui.set_min_height(220.0);
                                    self.ui_panel_container(cell_ui, idx);
                                });
                            }
                        });
                    });
...
fn main() {
    ...
    if let Err(e) = eframe::run_native(
        "dYdX CSV GUI + Rhai Bot (3x6 Grid)",
        options,
