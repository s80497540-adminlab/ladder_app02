// ladder_app/src/bin/gui_app16.rs
//
...
//   cargo run -p ladder_app --bin gui_app16
...
struct TradingState {
    ...
    // (real ticker now comes from MyApp.current_ticker)
    real_size_units: f64,
}
...
impl MyApp {
    ...
    fn ui_trading_panel(&mut self, ui: &mut egui::Ui, real_ticker: &str) {
        ui.group(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Ticker (locked to dropdown):");
    ...
    fn ui_candles(&mut self, ui: &mut egui::Ui) {
        ...
        ui.allocate_ui(egui::vec2(avail_w, candles_h), |ui| {
            ...
            Plot::new("candles_plot")
                ...
                .include_y(y_min)
                .include_y(y_max)
                .x_axis_formatter(move |x, _| format_ts_common(mode, x as u64))
                .show(ui, |plot_ui| {
...
fn main() {
    ...
    rt.spawn(async move {
        ...
        let mut sub_feed = match feeds.subaccounts(sub, false).await {
            Ok(f) => f,
            Err(e) => {
