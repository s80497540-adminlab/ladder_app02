// ladder_app/src/bin/full_gui_x04.rs
//
...
//   cargo run -p ladder_app --bin full_gui_x04
//
...
impl ComboApp {
    ...
    fn run_script(&mut self) {
        ...
        self.scope.set_value("tf", self.chart.selected_tf as i64);
        self.scope
            ...
            .eval_with_scope::<Dynamic>(&mut self.scope, &self.script)
        {
            Ok(_) => {
                ...
                if let Some(show_depth) = self.scope.get_value::<bool>("show_depth") {
                    self.layout.show_depth = show_depth;
                }
                if let Some(show_ladders) = self.scope.get_value::<bool>("show_ladders") {
                    self.layout.show_ladders = show_ladders;
                }
                if let Some(show_trades) = self.scope.get_value::<bool>("show_trades") {
                    self.layout.show_trades = show_trades;
                }
                if let Some(show_volume) = self.scope.get_value::<bool>("show_volume") {
                    self.layout.show_volume = show_volume;
                }

                if let Some(reload_secs) = self.scope.get_value::<f64>("reload_secs") {
                    if reload_secs > 0.05 {
                        self.reload_secs = reload_secs as f32;
                    }
                }
                ...
                ui.label(
                    RichText::new(
                        "You can tweak layout/appearance live.\n\
                         Available vars:\n\
                         - theme: string (\"Dark\", \"Light\", \"Ocean\", \"Fire\", \"Matrix\")\n\
                         ...
                         - reload_secs: f64 (CSV reload period)\n",
                    )
                    .color(self.theme.text),
                );
                ui.add(
...
fn main() {
    ...
    if let Err(e) = eframe::run_native(
        "dYdX CSV Viewer (daemon-powered) + Rhai",
        options,
