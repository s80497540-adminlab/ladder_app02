...
// dYdX client pieces – using the `dydx` crate from the workspace.
use dydx::config::ClientConfig;
use dydx::indexer::IndexerClient;
use dydx::node::{NodeClient, OrderBuilder, OrderSide, Wallet};
use dydx::node::order::TimeInForce;
...
// async trade executor – adapted to use `dydx`
async fn run_trader(mut rx: mpsc::Receiver<TradeCmd>) {
    let config =
        match ClientConfig::from_file("client/tests/testnet.toml").await {
            Ok(c) => c,
            Err(e) => {
    ...
    while let Some(cmd) = rx.recv().await {
            ...
            .await
        {
            ...
            Err(e) => {
                eprintln!(
                    "[trader] market meta error for {ticker}: {e}"
                );
                continue;
            }
        };
        ...
        if limit_price > 0.0 {
            // placeholder "price guard" wiring (you can refine later)
...
impl AppCore {
    fn new(
        ...
        for tk in &tickers {
            match load_ticker_data(&base_dir, tk) {
                Some(td) => {
                    println!(
                        "  {tk}: {} book events, {} trades, range {}..{}",
                        td.book_events.len(),
                        td.trade_events.len(),
                        td.min_ts,
                        td.max_ts
    ...
    fn reload_current_ticker(&mut self) {
        if let Some(td) = load_ticker_data(&self.base_dir, &self.current_ticker)
        {
            ...
            println!(
                "[RELOAD] {}: no CSV data found after reload",
                self.current_ticker
            );
        }
    }
        ...
        Ok(()) => {
                ...
                && new_size > 0.0
            {
                ...
                if let Some(side) = maybe_side {
                    ...
                    if let Ok(size_bd) = BigDecimal::fromStr(&size_str) {
...
fn update_ui_from_state(app: &AppWindow, core: &mut AppCore) {
    let Some(td) = core.ticker_data.get(&core.current_ticker) else {
        ...
        app.set_mid_price(0.0);
        app.set_best_bid(0.0);
        app.set_best_ask(0.0);
        app.set_spread(0.0);
        app.set_imbalance(0.0);
        app.set_bids(slint::ModelRc::new(
            slint::VecModel::from(Vec::<BookLevel>::new()),
        ));
        app.set_asks(slint::ModelRc::new(
            slint::VecModel::from(Vec::<BookLevel>::new()),
        ));
        app.set_recent_trades(slint::ModelRc::new(
            slint::VecModel::from(Vec::<Trade>::new()),
        ));
        app.set_receipts(slint::ModelRc::new(
            slint::VecModel::from(Vec::<Receipt>::new()),
        ));
        app.set_candles(slint::ModelRc::new(
            slint::VecModel::from(Vec::<CandleSample>::new()),
        ));
        app.set_candles_min_price(0.0);
        app.set_candles_max_price(0.0);
        app.set_candles_count(0);
        return;
    };
    ...
    if snap.candles.is_empty() {
        ...
        let window_len = core.history_candles.min(len).max(1);
        let start = len - window_len;
        let visible = &snap.candles[start..];
        ...
        let cand_vec: Vec<CandleSample> = visible
            .iter()
            .map(|c| CandleSample {
                t: c.t as i32,
                open: c.open as f32,
        ...
        app.set_candles_min_price(lo as f32);
        app.set_candles_max_price(hi as f32);
        app.set_candles_count(cand_vec.len() as i32);
        app.set_candles(slint::ModelRc::new(
            slint::VecModel::from(cand_vec),
        ));
    }
...
fn detect_base_dir() -> PathBuf {
    ...
    let candidates = [
        PathBuf::from("ladder_app/data"),
        PathBuf::from("data"),
        manifest_dir.join("../ladder_app/data"),
        manifest_dir.join("../data"),
    ];

    println!("Detecting CSV base dir...");
    for c in &candidates {
        let test_path = c.join("orderbook_ETH-USD.csv");
        if test_path.exists() {
            println!(
                "Using CSV base dir: {} (found {})",
                c.display(),
                test_path.display()
            );
            return c.clone();
        } else {
            println!("  candidate {} -> missing {}", c.display(), test_path.display());
        }
    }

    println!(
        "WARNING: no CSV candidate dir found (no orderbook_ETH-USD.csv); \
         defaulting to {}",
...
"#;
    ...
    // reload_data
    {
        ...
        app.on_reload_data(move || {
            let app = app_weak.unwrap();
            let mut core = core.borrow_mut();
            println!("[RELOAD button] Reloading data for {}", core.current_ticker);
    ...
    // run_script (manual)
    {
        ...
        app.on_run_script(move || {
            ...
            if let Some(td) = core.ticker_data.get(&core.current_ticker) {
                ...
                let snap =
                    compute_snapshot_for(td, target_ts, core.tf_secs);
    ...
    // periodic timer: reload + replay step + UI refresh
    {
        ...
        timer.start(
            ...
            move || {
                ...
                if now.saturating_sub(core.last_reload_ts)
                    >= core.reload_secs
                {
                    core.reload_current_ticker();
    ...
    println!("(CSV base dir auto-detected above)");
