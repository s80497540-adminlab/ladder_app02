...
impl Widget for CurrentRulerOverlay {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for x in area.x..(area.x + area.width) {
            if let Some(cell) = buf.cell_mut((x, row as u16)) {
                // don’t stomp candles too aggressively
                if cell.symbol() == " " || cell.symbol() == "─" {
...
/// Draw candles + RSI view (+ trading info + position overlay + current ruler)
fn draw_candles_view(
    ...
    if !series.is_empty() && inner.width > 0 && inner.height > 1 {
        ...
        f.render_widget(overlay, chart_area);
        ...
        f.render_widget(ruler_overlay, chart_area);
        ...
        if let (Some(first), Some(last)) = (visible.first(), visible.last()) {
            ...
            let axis_text = if chart.debug {
                Line::from(format!(
                    "t_start: {}   t_end(now): {}   (unix sec)   Δt_last={}s   y:[{:.2}, {:.2}]   N={}   now_px={:.2}",
                    first.t,
                    last.t,
                    dt_last,
                    y_min,
                ...
                Line::from(format!(
                    "t_start: {}   t_end(now): {}   (unix sec)   y:[{:.2}, {:.2}]   N={}   now_px={:.2}",
                    first.t,
                    last.t,
                    y_min,
                    y_max,
