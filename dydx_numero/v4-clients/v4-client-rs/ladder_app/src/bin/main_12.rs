...
impl TradingState {
    ...
    fn open_at(&mut self, mark: f64) {
        ...
        // if TP/SL were not set, keep them None; user sets manually
    }
        ...
        let draw_line = |price: f64,
            ...
            for x in area.x..(area.x + area.width) {
                if let Some(cell) = buf.cell_mut((x, r as u16)) {
                    // Only overwrite if it's empty or a grid line
...
/// Draw candles + RSI view (+ trading info + position overlay)
