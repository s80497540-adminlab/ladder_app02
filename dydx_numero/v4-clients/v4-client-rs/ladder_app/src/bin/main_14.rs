...
//   9 / 0 = margin -/+
//   - / = = position size -/+
//   [ / ] = leverage -/+ (clamped 1.00x..50.00x)
//   Enter = open/close fake position at current mark price
//
//   --- TP / SL controls (fake, ±1.0 price step) ---
//   u = move Take Profit up   (price + 1)
//   j = move Take Profit down (price - 1)
//   i = move Stop Loss up     (price + 1)
//   k = move Stop Loss down   (price - 1)
//
//   q = quit
...
struct TradingState {
    ...
    take_profit: Option<f64>, // TP price
    stop_loss: Option<f64>,   // SL price
}
...
impl TradingState {
    ...
    /// Adjust TP / SL relative to a base (current or existing).
    ...
    /// Check TP/SL and auto-close if hit.
...
/// Y-axis scale labels (price) aligned with the candle grid.
struct YAxisScale {
    y_min: f64,
    y_max: f64,
}
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for i in 0..=grid_lines {
            ...
            for (idx, ch) in label.chars().enumerate() {
                ...
                if let Some(cell) = buf.cell_mut((area.x + idx as u16, row as u16)) {
                    cell.set_symbol(&ch.to_string()).set_fg(Color::DarkGray);
                }
            }
        }
    }
...
/// Overlay for showing entry / TP / SL lines on the candles chart.
...
impl Widget for PositionOverlay {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let height = area.height as i32;
        ...
        let draw_line = |price: f64,
                         color: Color,
                         buf: &mut Buffer,
                         area: Rect,
                         row_min: i32,
                         row_max: i32,
                         map_price_to_row: &dyn Fn(f64) -> i32| {
            let r = map_price_to_row(price).clamp(row_min, row_max);
            for x in area.x..(area.x + area.width) {
                if let Some(cell) = buf.cell_mut((x, r as u16)) {
        ...
        // Entry line color depends on side
        if let Some(entry) = self.entry {
            let color = match self.side {
                PositionSide::Long => Color::Cyan,
                PositionSide::Short => Color::Magenta,
                PositionSide::Flat => Color::Yellow,
            };
            draw_line(entry, color, buf, area, row_min, row_max, &map_price_to_row);
        }

        if let Some(tp) = self.tp {
            draw_line(tp, Color::Green, buf, area, row_min, row_max, &map_price_to_row);
        }

        if let Some(sl) = self.sl {
            draw_line(sl, Color::Red, buf, area, row_min, row_max, &map_price_to_row);
        }
    }
}

/// Overlay for “ruler” at current price & current time (crosshair-ish).
struct CurrentRulerOverlay {
    price: f64,
    y_min: f64,
    y_max: f64,
    visible_len: usize,
...
impl CurrentRulerOverlay {
    fn new(price: f64, y_min: f64, y_max: f64, visible_len: usize) -> Self {
        Self {
            price,
            y_min,
            y_max,
...
impl Widget for CurrentRulerOverlay {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let height = area.height as i32;
        let width = area.width as usize;
        if height <= 0 || width == 0 || self.visible_len == 0 {
            return;
        }
        ...
        // horizontal at current price
        let row = map_price_to_row(self.price).clamp(row_min, row_max);
        for x in area.x..(area.x + area.width) {
            if let Some(cell) = buf.cell_mut((x, row as u16)) {
                if cell.symbol() == " " || cell.symbol() == "─" {
        ...
        // vertical at "now" (rightmost candle)
        let idx = self.visible_len.saturating_sub(1).min(width.saturating_sub(1));
        let col_x = area.x + idx as u16;

        for y in area.y..(area.y + area.height) {
            if let Some(cell) = buf.cell_mut((col_x, y)) {
...
fn main() -> Result<()> {
    ...
    loop {
        ...
        if event::poll(Duration::from_millis(1))? {
            if let Event::Key(k) = event::read()? {
                match k.code {
                    ...
                    // leverage [ / ]   (1.00x .. 50.00x)
                    KeyCode::Char('[') => {
                        trading.leverage = (trading.leverage - 0.5).max(1.0);
                    }
                    KeyCode::Char(']') => {
                        trading.leverage = (trading.leverage + 0.5).min(50.0);
                    }
                    ...
                    // u/j => TP up/down
                    KeyCode::Char('u') => {
                        trading.bump_tp(last_price, 1.0);
                    }
                    KeyCode::Char('j') => {
                        trading.bump_tp(last_price, -1.0);
                    }
                    ...
                    // i/k => SL up/down
                    KeyCode::Char('i') => {
                        trading.bump_sl(last_price, 1.0);
                    }
                    KeyCode::Char('k') => {
                        trading.bump_sl(last_price, -1.0);
                    }
        ...
        trading.check_tp_sl(last_price);
        ...
        terminal.draw(|f| {
            ...
            let tabs_line = {
                ...
                Paragraph::new(Line::from(vec![
                    ...
                    Span::raw(
                        "   ||  Trade: b=LONG s=SHORT f=FLAT  9/0=margin -/=pos [/]=lev u/j=TP i/k=SL (±1.0)"
                    ),
                    Span::raw(" | mode: "),
...
fn trading_widget(trading: &TradingState, mark_price: f64) -> Paragraph<'static> {
    ...
    let price_line = if trading.is_open() {
        format!(
            "Entry: {:>8.2} | Mark: {:>8.2} | TP: {:>8} | SL: {:>8} | uPnL: {:+9.2} | rPnL: {:+9.2} | Equity: ${:>9.2}",
            entry,
            mark_price,
            tp_str,
            sl_str,
        ...
        format!(
            "No open position. Mark: {:>8.2} | TP: {:>8} | SL: {:>8} | rPnL: {:+9.2} | Equity (cash): ${:>9.2}",
            mark_price,
            tp_str,
            sl_str,
            rpnl,
    ...
    let hint_line =
        "Enter=open/close  |  b=LONG s=SHORT f=FLAT  9/0=margin  -/=pos  [/]=lev  u/j=TP  i/k=SL (±1.0)";
...
/// Draw candles + RSI view (+ trading info + position overlay + current ruler + Y scale)
fn draw_candles_view(
    ...
    if !series.is_empty() && inner.width > 0 && inner.height > 1 {
        // split inner vertical into [chart; axis-row]
        ...
        // split chart horizontally into [y-axis; plot]
        let chart_h = Layout::default()
            .direction(Direction::Horizontal)
            .constraints([
                Constraint::Length(10), // Y-axis labels
                Constraint::Min(1),     // plot
            ])
            .split(chart_area);

        let y_axis_area = chart_h[0];
        let plot_area = chart_h[1];

        let window_len = chart
            .show_candles
            .min(plot_area.width as usize)
            .max(1);
        ...
        // Y-axis labels
        let y_axis = YAxisScale::new(y_min, y_max);
        f.render_widget(y_axis, y_axis_area);

        // candles
        let chart_widget = CandleChart::new(&visible, y_min, y_max);
        f.render_widget(chart_widget, plot_area);

        // position overlay (entry / TP / SL)
        let overlay = PositionOverlay::new(
            trading.entry_price,
            trading.take_profit,
            trading.stop_loss,
            y_min,
        ...
        f.render_widget(overlay, plot_area);

        // "ruler" overlay: current price + current time (rightmost candle)
        let now_price = visible.last().map(|c| c.close).unwrap_or(mark_price);
        let ruler_overlay =
            CurrentRulerOverlay::new(now_price, y_min, y_max, visible.len());
        f.render_widget(ruler_overlay, plot_area);
        ...
        if let (Some(first), Some(last)) = (visible.first(), visible.last()) {
            ...
            let axis_text = if chart.debug {
                Line::from(format!(
                    "Time axis: t_start={}   t_end(now)={}   (unix sec)   Δt_last={}s   y:[{:.2}, {:.2}]   N={}   now_px={:.2}",
                    first.t,
                    last.t,
                    dt_last,
                    y_min,
                ...
                Line::from(format!(
                    "Time axis: t_start={}   t_end(now)={}   (unix sec)   y:[{:.2}, {:.2}]   N={}   now_px={:.2}",
                    first.t,
                    last.t,
                    y_min,
                    y_max,
