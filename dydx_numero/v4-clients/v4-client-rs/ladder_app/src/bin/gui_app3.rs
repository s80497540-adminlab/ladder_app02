...
impl MyApp {
    ...
    fn ui_trading_panel(&mut self, ui: &mut egui::Ui) {
        ui.group(|ui| {
            ...
            ui.add(
                egui::Slider::new(&mut self.trading.margin, 0.0..=10_000.0)
                    .text("Margin (USD)"),
            );
            ui.add(
            ...
            ui.add(
                egui::Slider::new(&mut self.trading.position, 0.0..=100.0)
                    .text("Position (units)"),
            );
    ...
    fn ui_candles(&mut self, ui: &mut egui::Ui) {
        ...
        ui.allocate_ui(egui::vec2(avail_w, candles_h), |ui| {
            Plot::new("candles_plot")
                ...
                .show(ui, |plot_ui| {
                    ...
                    // Candles on real time x, bodies span full bucket width
                    for c in visible {
                        let left = c.t as f64;
                        let right = left + tf;
                        let mid = left + tf * 0.5;
        ...
        ui.allocate_ui(egui::vec2(avail_w, rsi_h), |ui| {
            ...
            // map RSI points to candle times
