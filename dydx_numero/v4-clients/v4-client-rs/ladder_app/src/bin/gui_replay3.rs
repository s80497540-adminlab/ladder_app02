...
impl ReplayApp {
    ...
    fn ui_data(&mut self, ui: &mut egui::Ui) {
        ...
        egui::CollapsingHeader::new("Recent trades around current time")
            .default_open(true)
            .show(ui, |ui| {
                let window_secs: u64 = 120; // 2 minutes
                ...
                if rows.is_empty() {
                    ui.label("No trades in the last 2 minutes of replay time.");
                    return;
                }
        ...
        egui::CollapsingHeader::new("Recent raw orderbook events")
            ...
            .show(ui, |ui| {
                let max_rows = 80;
                let start = self.ob_index.saturating_sub(max_rows);
                let slice = &self.ob_events[start..self.ob_index];

                if slice.is_empty() {
                    ui.label("No orderbook events yet at this replay point.");
                    return;
                }
